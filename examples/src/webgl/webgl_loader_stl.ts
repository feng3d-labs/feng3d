...
import { loadSTLFromUrl } from '@feng3d/addons';

/**
 * 展示 STLLoader：加载 slotted_disk.stl（ASCII STL），旋转展示。
 */

const webgpuCanvas = document.getElementById('webgpu') as HTMLCanvasElement;
const webgpu = await new WebGPU().init();

const geometry = await loadSTLFromUrl('/slotted_disk.stl');
...
const view: View = {
    ...
    root: {
        ...
        children: [
            {
                __type__: 'Object3D', name: 'Main Camera', position: { x: 0, y: -2, z: 4 },
                components: [
            ...
            {
                __type__: 'Object3D', name: 'mesh', scale: { x: 5, y: 5, z: 5 }, rotation: meshRot = { x: 0, y: 0, z: 0 },
                components: [{
                    __type__: 'MeshRenderer', geometry,
                    material: {
                        __type__: 'StandardMaterial',
                        uniforms: {
                            u_diffuse: { __type__: 'Color4', r: 0.8, g: 0.8, b: 0.85, a: 1 },
