...
const SEG = 32;
function buildSphere(r: number): CustomGeometry { const pos: number[]=[]; const nor: number[]=[]; const uv: number[]=[]; const idx: number[]=[]; const col: number[]=[];
  ...
  const g: CustomGeometry = { __type__: 'CustomGeometry' }; const gl=logic(g); gl.positions=pos; gl.normals=nor; gl.uvs=uv; gl.colors=col; (gl as unknown as {indices:number[]}).indices=idx; return g; }
...
ticker.onframe(() => { const t = Date.now()*0.001; const r = 1.5+Math.sin(t*2)*0.5; curGeo = buildSphere(r); reactive(morphNode.components![0] as object).geometry = curGeo; reactive(morphNode.rotation as {y:number}).y = t*0.3; webgpu.submit(vl.submit); });
