...
const controller = new LookAtController(logic(camera).object3D);
...
setInterval(() => {
    ...
    { const _r = reactive(logic(camera).object3D.position); _r.x = 10 * Math.sin(angle); _r.y = 0; _r.z = 10 * Math.cos(angle); }
