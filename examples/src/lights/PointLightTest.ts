import { batchRun, Camera, Color4, CubeGeometry, ColorMaterial, DirectionalLight, FPSController, Object3D, PlaneGeometry, PointLight, reactive, Renderable, Scene, ShadowType, SphereGeometry, StandardMaterial, Texture2D, TextureWrap, ticker, transformLogic, Transform, Vector3, View, windowEventProxy, object3DLogic } from 'feng3d';
...
function initObjects() {
    ...
    const model = object3DLogic(plane).addComponent(Renderable);
...
function initLights() {
    ...
    let model = object3DLogic(light0).addComponent(Renderable);
    const sphereGeo0 = new SphereGeometry(); sphereGeo0.radius = 0.05;
    model.geometry = sphereGeo0;
    //初始化点光源
    const pointLight0 = object3DLogic(light0).addComponent(PointLight);
    ...
    model = object3DLogic(light1).addComponent(Renderable);
    const sphereGeo1 = new SphereGeometry(); sphereGeo1.radius = 0.05;
    model.geometry = sphereGeo1;
    //初始化点光源
    const pointLight1 = object3DLogic(light1).addComponent(DirectionalLight);
...
function setPointLightPosition() {
    ...
    lookAtTransform(light1.transform, new Vector3());
}
