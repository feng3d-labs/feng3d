...
- [x] **class 化全部完成（2026-08-15）**：剩余批次已清零——光源子类 ×3（f04d6db7）、相机子类 ×2（2755d5f6）、Geometry 基类 + Custom/Point/Segment + primitives ×8 + Cone 复用（ad8bc1bd）、Animation/Audio ×2/FPSController/OrbitControls（57399035）、核心链 Entity→Container→Object3D 继承链 + Renderable/MeshRenderer/SkinnedMeshRenderer/View/Scene + SceneUtil/ParticleSystem（89817dbf）、addons 几何体 ×14 + terrain ×2（b2696d68，多面体族改继承 PolyhedronGeometryLogic）。registerLogic 全仓无小写工厂函数残留（Geometry 基类保留泛型包装以通过弱类型检查）。
...
- [x] $ref 共享引用（构造侧 b5aec891+abcde6d9）；[x] 序列化反向提升（保存侧 c2050383：liftSharedRefs 多处引用自动提升到 shared 表、注册表对象还原原键；View.defs 双注册补全路径语法）
