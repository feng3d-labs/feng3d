namespace feng3d
{
    QUnit.module("TriangleGeometry", () =>
    {
        ...
        {
            // var box = Box.random();
            var box = new Box3(new Vector3(), new Vector3(1, 1, 1));
            var triangleGeometry = TriangleGeometry.fromBox(box);
        ...
        QUnit.test("intersectionWithSegment", (assert) =>
        {
            ...
            var r = triangleGeometry.intersectionWithSegment(Segment3.fromPoints(box.min, box.max))
            assert.ok(r);
            if (r)
            {
            ...
            if (r1)
            {
                ...
                assert.ok(Segment3.fromPoints(p0, new Vector3(box.min.x, box.min.y, box.max.z)).equals(r1.segments[0]));
            }
        });
    });
}
