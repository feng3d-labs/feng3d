...
interface LoadItem<T = string | ArrayBuffer>
{
    url: string;
    dataFormat: LoaderDataFormat;
    onCompleted?: (content: T | null) => void;
 ...
 */
function xmlHttpRequestLoad<T>(loadItem: LoadItem<T>)
{
 ...
 * 请求进度回调
 */
function onRequestProgress<T>(request: XMLHttpRequest, loadItem: LoadItem<T>)
{
 ...
 * 请求状态变化回调
 */
function onRequestReadystatechange<T>(request: XMLHttpRequest, loadItem: LoadItem<T>)
{
    return (_ev) =>
    {
        ...
        { // 4 = "loaded"
            ...
            {
                const content: T = (loadItem.dataFormat === LoaderDataFormat.TEXT ? request.responseText : request.response) as T;
