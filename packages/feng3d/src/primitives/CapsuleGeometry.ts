...
export class CapsuleGeometryLogic extends GeometryLogic
{
    ...
    readonly #radius = (): number => reactive(this._data as CapsuleGeometry).radius ?? 0.5;
    readonly #height = (): number => reactive(this._data as CapsuleGeometry).height ?? 1;
    readonly #segmentsW = (): number => reactive(this._data as CapsuleGeometry).segmentsW ?? 16;
    readonly #segmentsH = (): number => reactive(this._data as CapsuleGeometry).segmentsH ?? 15;
    readonly #yUp = (): boolean => reactive(this._data as CapsuleGeometry).yUp ?? true;
    ...
    protected constructor(data: CapsuleGeometry)
    {
        super(data);
    }

    /** 内部创建入口（protected constructor 的唯一出口） */
    static create(data: CapsuleGeometry): CapsuleGeometryLogic
    {
        return new CapsuleGeometryLogic(data);
    }
    ...
    #buildPositions(): Float32Array
    {
        ...
        for (let yi = 0; yi <= this.#segmentsH(); ++yi)
        {
            ...
            for (let xi = 0; xi <= this.#segmentsW(); ++xi)
            {
                ...
                {
                    data[index] = x;
                    data[index + 1] = this.#yUp() ? comp1 - offset : comp1;
                    data[index + 2] = this.#yUp() ? comp2 : comp2 + offset;
                }
