...
const links: Record<string, HTMLDivElement> = {};
const validRedirects = new Map<string, string>();
...
function createLink(file: string, tags: unknown)
{
...
function selectFile(file: string)
{
    ...
    viewSrcButton.href = `https://gitee.com/feng3d/feng3d/tree/master/packages/rendering/examples/src/webgpu/${selected}/index.ts`;
...
function updateFilter(files: unknown, tags: unknown)
{
    ...
    for (const key in files as Record<string, unknown>)
    {
        const section = (files as Record<string, string[]>)[key];
...
function updateLink(search: string)
{
    // update docs link
...
function filterExample(file: string, exp: RegExp, tags: unknown)
{
    const link = links[file];

    if (!link) return;
    if (file in (tags as Record<string, unknown>)) file += ` ${(tags as Record<string, string[]>)[file].join(' ')}`;
...
function layoutList(files: unknown)
{
    for (const key in files as Record<string, unknown>)
    {
        let collapsed = true;
