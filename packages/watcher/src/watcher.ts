export class Watcher
{
     ...
     */
    watch<T extends Record<string, unknown>>(object: T, property: string, handler: WatchHandler, thisObject?: unknown): void;
    watch<T, K extends PropertyNames<T>, V extends T[K]>(object: T, property: K, handler: (newValue: V, oldValue: V, object: T, property: string) => void, thisObject?: unknown): void;
    watch<T, K extends PropertyNames<T> | string, V>(object: T, property: K, handler: (newValue: V, oldValue: V, object: T, property: string) => void, thisObject?: unknown)
    {
        ...
        const watchs = object[__watchs__] as Watchs;
     ...
     * @param thisObject 变化回调函数 this值
     */
    unwatch<T extends Record<string, unknown>>(object: T, property: string, handler?: WatchHandler, thisObject?: unknown): void;
    unwatch<T, K extends PropertyNames<T>, V extends T[K]>(object: T, property: K, handler?: (newValue: V, oldValue: V, object: T, property: string) => void, thisObject?: unknown): void;
    unwatch<T, K extends PropertyNames<T> | string, V>(object: T, property: K, handler?: (newValue: V, oldValue: V, object: T, property: string) => void, thisObject?: unknown)
    {
        const watchs = object[__watchs__] as Watchs | undefined;
    ...
    bind<T0, T1, K0 extends PropertyNames<T0>, K1 extends PropertyNames<T1>>(object0: T0, property0: K0, object1: T1, property1: K1)
    {
        const fun0 = () =>
        {
            (object1 as Record<string, unknown>)[property1 as string] = object0[property0];
        };
        const fun1 = () =>
        {
            (object0 as Record<string, unknown>)[property0 as string] = object1[property1];
        };

        this.watch(object0, property0, fun0);
        this.watch(object1, property1, fun1);

        this._binds.push([object0 as Record<string, unknown>, property0 as string, fun0, object1 as Record<string, unknown>, property1 as string, fun1]);
    }
     ...
     * @param handler 变化回调函数 (newValue, oldValue, object, property) => void
     * @param thisObject 变化回调函数 this值
     */
    watchchain(object: WatchableObject, property: string, handler: WatchHandler, thisObject?: unknown)
    {
        ...
        const watchchains = object[__watchchains__] as WatchChains;
        ...
        if (!has)
        {
            ...
            const currentValue = object[currentp] as WatchableObject;
            if (currentValue)
            {
                this.watchchain(currentValue, nextp, handler, thisObject);
            }

            // 添加链监听
            const watchchainFun: WatchHandler = (newValue, oldValue) =>
            {
                if (oldValue) this.unwatchchain(oldValue as WatchableObject, nextp, handler, thisObject);
                if (newValue) this.watchchain(newValue as WatchableObject, nextp, handler, thisObject);
     ...
     * @param thisObject 变化回调函数 this值
     */
    unwatchchain(object: WatchableObject, property: string, handler?: WatchHandler, thisObject?: unknown)
    {
        const notIndex = property.indexOf('.');
        ...
        for (let i = propertywatchs.length - 1; i >= 0; i--)
        {
            ...
            if (objectIsEmpty(handler) || (handler === element.handler && thisObject === element.thisObject))
            {
                ...
                if (currentValue)
                {
                    this.unwatchchain(currentValue, nextp, element.handler, element.thisObject);
                }
                // 删除链监听
     ...
     * @param handler 变化回调函数 (newValue, oldValue, host, property) => void
     * @param thisObject 变化回调函数 this值
     */
    watchobject<T>(object: T, property: gPartial<T>, handler: WatchHandler, thisObject?: unknown)
    {
        const chains = getObjectPropertyChains(property as object);

        chains.forEach((v) =>
        {
            this.watchchain(object as unknown as WatchableObject, v, handler, thisObject);
        });
    }
     ...
     * @param thisObject 变化回调函数 this值
     */
    unwatchobject<T>(object: T, property: gPartial<T>, handler?: WatchHandler, thisObject?: unknown)
    {
        const chains = getObjectPropertyChains(property as object);

        chains.forEach((v) =>
        {
            this.unwatchchain(object as unknown as WatchableObject, v, handler, thisObject);
        });
    }
}
 ...
 * @param property 属性名称
 */
export type WatchHandler = (newValue: unknown, oldValue: unknown, host: unknown, property: string) => void;

/**
 * 可被监听的对象：允许通过字符串键索引读写属性
 */
export type WatchableObject = Record<string, unknown>;

/**
 * 单条属性绑定记录：[对象0, 属性0, 回调0, 对象1, 属性1, 回调1]
 */
export type BindRecord = [WatchableObject, string, WatchHandler, WatchableObject, string, WatchHandler];

interface Watchs
{
    [property: string]: {
        value: unknown,
        oldPropertyDescriptor: PropertyDescriptor | undefined,
        handlers: { handler: WatchHandler, thisObject: unknown }[]
    };
}

interface WatchChains
{
    [property: string]: { handler: WatchHandler, thisObject: unknown, watchchainFun: WatchHandler }[];
}

export const __watchs__ = '__watchs__';
export const __watchchains__ = '__watchchains__';

function notifyListener(newValue: unknown, oldValue: unknown, host: WatchableObject, property: string): void
{
    const watchs = host[__watchs__] as Watchs;
 ...
 * @returns
 */
function objectIsEmpty(obj: unknown)
{
 ...
 */
function getObjectPropertyValue(object: unknown, property: string | string[]): unknown
{
...
function getObjectPropertyChains(object: object): string[]
{
    ...
    // 属性所属对象列表
    const hosts: unknown[] = new Array(propertys.length).fill(object);
    // 父属性所在编号列表
    const parentPropertyIndices: number[] = new Array(propertys.length).fill(-1);
    // 处理到的位置
    let index = 0;
    while (index < propertys.length)
    {
        const host = hosts[index] as Record<string, unknown>;
        const cp = propertys[index];
        const cv = host[cp];
        let vks: string[];
        if (objectIsEmpty(cv) || isBaseType(cv) || (vks = Object.keys(cv as object)).length === 0)
        {
            // 处理叶子属性
            const ps = [cp];
            let ci = index;
 ...
 */
function isBaseType(object: unknown): boolean
{
    // 基础类型
    if (
