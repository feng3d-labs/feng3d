...
export class WGPURenderPassColorAttachment extends ReactiveObject
{
    ...
    private _onCreate(device: GPUDevice, colorAttachment: RenderPassColorAttachment, descriptor: RenderPassDescriptor, canvasContext?: CanvasContext)
    {
        ...
        // 颜色附件对象在内部 computed 中逐步填充必填字段（loadOp/storeOp/view），
        // 因此先以 Partial 形式累积，最终在返回前断言为完整类型。
        const gpuRenderPassColorAttachment: Partial<GPURenderPassColorAttachment> = {};
        ...
        this._computedGpuRenderPassColorAttachment = computed(() =>
        {
            ...
            // 更新颜色附件引用
            // 子 computed 已经设置了 loadOp/storeOp/view 等必填字段，
            // 此处断言为完整类型（非 any，仅做 Partial -> 完整类型的收敛）。
            return gpuRenderPassColorAttachment as GPURenderPassColorAttachment;
        });
