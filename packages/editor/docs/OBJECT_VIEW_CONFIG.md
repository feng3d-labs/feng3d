...
| `OBVInline` | 字段**挤在同一行**、等宽平分、**不折叠也不换行**；标题占行首一窄列 | 名称 / 标签 / 启用 / 可拾取 这类短字段，一眼扫过即可 |

```ts
Object3D: {
    blocks: [
        { name: '基本信息', component: 'OBVInline' },   // ← 一行装下四个短字段
...
- 字段控件原本是「标签占 120px + 值占剩余」的整行布局，一行塞四个时横排标签会把输入框挤没，
  所以它把**标签改到控件上方**（`:deep(.oav-row)` 覆盖——字段 DOM 是命令式挂载的，必须用 `:deep`）。
- 面板宽度小于 320px 时用**容器查询**让行首标题让位：标题是给自己看的，输入框太窄就没法用了。
  无论如何都保持一行——「同一行」是这个块视图的语义。
