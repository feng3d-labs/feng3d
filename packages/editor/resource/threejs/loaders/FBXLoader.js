...
( function () {
	...
	function parseScene( FBXTree, connections, deformers, geometryMap, materialMap ) {
		...
		if ( BindPoseNode ) {
			...
			for ( var PoseNodeIndex = 0, PoseNodeLength = PoseNode.length; PoseNodeIndex < PoseNodeLength; ++ PoseNodeIndex ) {
				...
				{
					node.id = parseInt(node.properties.Node);
				}
				...
				for ( var bonesIndex = 0, bonesLength = bones.length; bonesIndex < bonesLength; ++ bonesIndex ) {
					...
					for ( var hierarchyIndex = 0, hierarchyLength = animationData.hierarchy.length; hierarchyIndex < hierarchyLength; ++ hierarchyIndex ) {
						...
						if ( node.name === bone.name ) {
							...
							fixedAnimations(animationNode,bone,stack.frames,stack.length);
	...
	function fixedBones(deformers)
	{
		for (var key in deformers) {
			if (deformers.hasOwnProperty(key)) {
				var boneNames = {};
				var bones = deformers[key].bones;
				for (var i = 0; i < bones.length; i++) {
					var bone = bones[i];
					if(bone.name == "")
					{
						console.assert(bones[i].FBX_ID === undefined)
						bone = bone.parent;
					}
					//同名骨骼修改名称
					if(boneNames[bone.name])
					{
						bone.name = getNewName(bone.name);
					}
	...
	function fixedAnimations(animationNode,bone,frameNum,length)
	{
		if(!animationNode || animationNode.isfixed)
			return;
		animationNode.isfixed = true;

		var fps = frameNum/length;

		var defaultpos = bone.position.toArray();
		var defaultrot = [bone.rotation.x,bone.rotation.y,bone.rotation.z];
		var defaultscl = bone.scale.toArray();
		//
		if(animationNode.T)
		{
			var tx = animationNode.T.curves.x = animationNode.T.curves.x ||{times:[],values:[]};
			var ty = animationNode.T.curves.y = animationNode.T.curves.y ||{times:[],values:[]};
			var tz = animationNode.T.curves.z = animationNode.T.curves.z ||{times:[],values:[]};
			initCurvesItem(tx,fps,frameNum,defaultpos[0]);
			initCurvesItem(ty,fps,frameNum,defaultpos[1]);
			initCurvesItem(tz,fps,frameNum,defaultpos[2]);
		}
		//
		if(animationNode.R)
		{
			var rx = animationNode.R.curves.x = animationNode.R.curves.x ||{times:[],values:[]};
			var ry = animationNode.R.curves.y = animationNode.R.curves.y ||{times:[],values:[]};
			var rz = animationNode.R.curves.z = animationNode.R.curves.z ||{times:[],values:[]};
			initCurvesItem(rx,fps,frameNum,defaultrot[0]);
			initCurvesItem(ry,fps,frameNum,defaultrot[1]);
			initCurvesItem(rz,fps,frameNum,defaultrot[2]);
		}
		//
		if(animationNode.S)
		{
			var sx = animationNode.S.curves.x = animationNode.S.curves.x ||{times:[],values:[]};
			var sy = animationNode.S.curves.y = animationNode.S.curves.y ||{times:[],values:[]};
			var sz = animationNode.S.curves.z = animationNode.S.curves.z ||{times:[],values:[]};
			initCurvesItem(sx,fps,frameNum,defaultscl[0]);
			initCurvesItem(sy,fps,frameNum,defaultscl[1]);
			initCurvesItem(sz,fps,frameNum,defaultscl[2]);
		}
	}

	function initCurvesItem(curvesItem,fps,frameNum,defaultvalue)
	{
		if(!curvesItem)
			return;
		var times = curvesItem.times;
		var values = curvesItem.values;
		curvesItem.times = [];
		curvesItem.values = [];
		...
		{
			var time = i/fps;
			curvesItem.times[i] = time;
			curvesItem.values[i] = defaultvalue;
			...
			if(time > times[times.length - 1])
				continue;
			while(times[timeindex] < time)
			{
				timeindex++;
			}
			if(times[timeindex] == time)
				curvesItem.values[i] = values[timeindex];
			else
				curvesItem.values[i] = (values[timeindex-1] *(times[timeindex] - time) +  values[timeindex] * (time - times[timeindex-1]))/(times[timeindex] - times[timeindex-1]);
		}
	}
