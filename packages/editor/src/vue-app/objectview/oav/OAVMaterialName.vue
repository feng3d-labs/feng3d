...
// Shader 名称（Material 重构后 shader 在构造时固定，此处通过 any 读取旧字段）
const shaderName = computed(() => (material.value as any)?.shaderName || '');

// Shader 选项列表（shaderlib 已移除，返回空列表）
const shaderOptions = computed<{ label: string; value: string }[]>(() => {
    return [];
});
