...
export class MRSToolBase extends Component
{
    ...
    private update()
    {
        const holdSizeComponent = this.object3D.getComponent(HoldSizeComponent);
    ...
    {
        if (this._toolModel)
        { this.object3D.removeChild(this._toolModel.object3D); }
        this._toolModel = value;
        if (this._toolModel)
        {
            this.object3D.addChild(this._toolModel.object3D);
        }
    }
    ...
    {
        const line3D = this.object3D.scene.mouseRay3D;
        // 射线与平面交点
