...
export class MTool extends MRSToolBase
{
    ...
    init()
    {
        ...
        this.toolModel = new Object3D().addComponent(MToolModel);
    }
    ...
    protected onItemMouseDown(event: IEvent<any>)
    {
        ...
        const sp = this.toolModel.transform.position;
        this.startPos = new Vector3(sp.x, sp.y, sp.z);
    ...
    protected updateToolModel()
    {
...

        {
            ...
            r.x = localCameraPos.x > 0 ? 0 : -this.toolModel.xyPlane.width;
            r.y = localCameraPos.y > 0 ? 0 : -this.toolModel.xyPlane.width;
        }

        {
            const r = reactive(this.toolModel.xzPlane.transform.position);
            r.x = localCameraPos.x > 0 ? 0 : -this.toolModel.xzPlane.width;
            r.z = localCameraPos.z > 0 ? 0 : -this.toolModel.xzPlane.width;
        }

        {
            const r = reactive(this.toolModel.yzPlane.transform.position);
            r.y = localCameraPos.y > 0 ? 0 : -this.toolModel.yzPlane.width;
            r.z = localCameraPos.z > 0 ? 0 : -this.toolModel.yzPlane.width;
        }
    }
}
