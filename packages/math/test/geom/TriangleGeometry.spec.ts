...
describe('TriangleGeometry', () =>
{
    it('fromBox,getBox', () =>
    {
        ...
        const triangleGeometry = TriangleGeometry.fromBox(box);
    ...
    it('intersectionWithSegment', () =>
    {
        ...
        const r = triangleGeometry.intersectionWithSegment(Segment3.fromPoints(box.min, box.max));
        ok(r);
        if (r)
        {
            ok(r.segments.length === 0);
            ok(r.points.length === 2);
            ok(Segment3.fromPoints(r.points[0], r.points[1]).equals(Segment3.fromPoints(box.min, box.max)));
        }
        ...
        const s = Segment3.fromPoints(p0, p1);
        ...
        if (r1)
        {
            ...
            ok(Segment3.fromPoints(p0, new Vector3(box.min.x, box.min.y, box.max.z)).equals(r1.segments[0]));
        }
    });
});
