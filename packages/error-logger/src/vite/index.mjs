...
//
// 一个插件同时完成两件事：
// 1. 服务端：注册 /api/log 中间件，接收前端上报的日志并写入本地文件
...
// 虚拟模块 ID：transformIndexHtml 注入的 script src 指向它，由 resolveId/load 提供内容。
 ...
 *
 * 拦截 console.log/warn/error/info 与全局 error/unhandledrejection，
 * 通过 sendBeacon 上报到 endpoint（默认 /api/log）。
 */
function clientInitScript(endpoint) {
    return `
...
window.addEventListener('error', e => console.error('[全局错误] ' + e.message + ' at ' + e.filename + ':' + e.lineno + ':' + e.colno, e.error));
window.addEventListener('unhandledrejection', e => console.error('[未捕获Promise] ' + (e.reason instanceof Error ? e.reason.message : String(e.reason)), e.reason));
 ...
 * @param {string} [options.endpoint='/api/log'] 日志接收端点路径
 * @param {string} [options.logDir] 日志输出目录（默认 <项目根>/logs）
 ...
 */
export function errorLoggerPlugin(options) {
    const opts = options || {};
    const endpoint = opts.endpoint ?? '/api/log';
    const rawLogDir = opts.logDir;

    const resolveLogDir = (viteRoot) => {
            ...
            server.middlewares.use(endpoint, (req, res) => {
                ...
                req.on('end', () => {
                    try {
                        ...
                        // clientId 由前端生成：'client_<页面加载时的Date.now()>'，含毫秒可区分不同会话。
