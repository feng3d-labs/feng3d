...
export class EquationSolving
{
     ...
     */
    binary(f: (x: number) => number, a: number, b: number, precision = 0.0000001, errorcallback?: (err: Error) => void): number
    {
     ...
     * @returns 不存在解时返回 undefined ，存在时返回 解
     */
    line(f: (x: number) => number, a: number, b: number, precision = 0.0000001, errorcallback?: (err: Error) => void): number
    {
        if (!this.hasSolution(f, a, b, errorcallback)) return undefined;
     ...
     */
    tangent(f: (x: number) => number, f1: (x: number) => number, f2: (x: number) => number, a: number, b: number, precision = 0.0000001, errorcallback?: (err: Error) => void): number
    {
        if (!this.hasSolution(f, a, b, errorcallback)) return undefined;
     ...
     * @returns 不存在解与无法使用该函数求解时返回 undefined ，否则返回 解
     */
    secant(f: (x: number) => number, a: number, b: number, precision = 0.0000001, errorcallback?: (err: Error) => void): number
    {
        if (!this.hasSolution(f, a, b, errorcallback)) return undefined;
