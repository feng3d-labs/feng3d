 ...
 *
 * 注：类型参数 `O` 约束为 `IEventTarget` 以表达事件目标的静态契约；运行时仍支持任意非空对象与基础类型，
 * 传入基础类型等非 `IEventTarget` 形态的值时需由调用方自行确保类型安全。
 */
export class AnyEmitter<O extends IEventTarget = IEventTarget, T = any>
{
    /**
     ...
     */
    once<K extends keyof T & string>(obj: O, type: K, listener: (event: IEvent<T[K]>) => void, thisObject: unknown = null, priority = 0)
    {
        console.assert(obj !== undefined && obj !== null, `被监听对象无法为undefined或者null！`);
     ...
     */
    on<K extends keyof T & string>(obj: O, type: K, listener: (event: IEvent<T[K]>) => void, thisObject?: unknown, priority = 0, once = false)
    {
        console.assert(obj !== undefined && obj !== null, `被监听对象无法为undefined或者null！`);
     ...
     * @param thisObject                监听器的上下文。可选。
     */
    off<K extends keyof T & string>(obj: O, type?: K, listener?: (event: IEvent<T[K]>) => void, thisObject?: unknown)
    {
        console.assert(obj !== undefined && obj !== null, `被监听对象无法为undefined或者null！`);
     ...
     * @param once                      值为true时在监听一次事件后该监听器将被移除。默认为false。
     */
    onAny<K extends keyof T & string>(obj: O, listener: (event: IEvent<T[K]>) => void, thisObject?: unknown, priority = 0, once = false)
    {
        console.assert(obj !== undefined && obj !== null, `被监听对象无法为undefined或者null！`);
     ...
     * @param thisObject                监听器的上下文。可选。
     */
    offAny<K extends keyof T & string>(obj: O, listener?: (event: IEvent<T[K]>) => void, thisObject?: unknown)
    {
        console.assert(obj !== undefined && obj !== null, `被监听对象无法为undefined或者null！`);
