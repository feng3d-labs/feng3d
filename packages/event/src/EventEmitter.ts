 ...
 * 事件发射器
 *
 * 类型参数 `T` 为事件名到事件数据的映射（EventMap）；默认值为 `any`，表示开放的事件映射，
 * 允许以任意字符串作为事件名派发事件，以便与 `objectEmitter` 等具体事件映射的发射器共享同一单例实例。
 */
export class EventEmitter<T = any> implements IEventTarget
{
    /**
     * 获取分享的平级目标列表。
     *
     * 此处仅为类型声明（`declare`，运行时不存在该属性），由子类或具体目标对象按需提供实现；
     * 未提供时事件流内部不会进行平级分享。
     */
    declare getShareTargets?: () => IEventTarget[];

    /**
     * 获取报告的上级目标列表。
     *
     * 此处仅为类型声明（`declare`，运行时不存在该属性），由子类或具体目标对象按需提供实现；
     * 未提供时事件流内部按目标对象的 `parent` 属性回退处理。
     */
    declare getBubbleTargets?: () => IEventTarget[];

    /**
     * 获取广播的下级目标列表。
     *
     * 此处仅为类型声明（`declare`，运行时不存在该属性），由子类或具体目标对象按需提供实现；
     * 未提供时事件流内部按目标对象的 `children` 属性回退处理。
     */
     ...
     * @param target
     */
    static getEventEmitter(target: IEventTarget)
    {
     ...
     * @param target
     */
    static getOrCreateEventEmitter(target: IEventTarget)
    {
     ...
     */
    once<K extends keyof T & string>(type: K, listener: (event: IEvent<T[K]>) => void, thisObject?: unknown, priority = 0): this
    {
     ...
     */
    on<K extends keyof T & string>(type: K, listener: (event: IEvent<T[K]>) => void, thisObject?: unknown, priority = 0, once = false): this
    {
     ...
     * @param thisObject                监听器的上下文。可选。
     */
    off<K extends keyof T & string>(type?: K, listener?: (event: IEvent<T[K]>) => void, thisObject?: unknown): this
    {
        if (!type)
        {
     ...
     * @param once                      值为true时在监听一次事件后该监听器将被移除。默认为false。
     */
    onAny<K extends keyof T & string>(listener: (event: IEvent<T[K]>) => void, thisObject?: unknown, priority = 0, once = false)
    {
        let objectListener = EventEmitter.emitterListenerMap.get(this);
     ...
     * @param thisObject                监听器的上下文。可选。
     */
    offAny<K extends keyof T & string>(listener?: (event: IEvent<T[K]>) => void, thisObject?: unknown)
    {
        const objectListener = EventEmitter.emitterListenerMap.get(this);
