...
function createInstrumentations(): Instrumentations
{
    const instrumentations: Instrumentations = {
        ...
        get(this: MapTypes, key: unknown)
        {
            ...
            const target = this[ReactiveFlags.RAW] as MapTypes;
        ...
        {
            const target = this[ReactiveFlags.RAW] as CollectionTypes;
            const rawTarget = toRaw(target);
            const rawKey = toRaw(key);
        ...
        {
            const observed = this;
            const target = observed[ReactiveFlags.RAW] as IterableCollections;
            const rawTarget = toRaw(target);
            const wrap = toReactive;
...
function createIterableMethod(method: string | symbol)
{
    ...
    {
        const target = this[ReactiveFlags.RAW] as IterableCollections;
        const rawTarget = toRaw(target);
        const targetIsMap = isMap(rawTarget);
        const isPair
            = method === 'entries' || (method === Symbol.iterator && targetIsMap);
        const isKeyOnly = method === 'keys' && targetIsMap;
        const innerIterator = (target[method] as (this: IterableCollections, ...args: unknown[]) => Iterator<unknown | [unknown, unknown]>).apply(target, args);
        const wrap = toReactive;
        ...
        return {
            ...
            next()
            {
                ...
                return done
                    ? { value, done }
                    : {
                        value: isPair ? [wrap((value as [unknown, unknown])[0]), wrap((value as [unknown, unknown])[1])] : wrap(value),
                        done,
                    };
            },
            // 可迭代协议
 ...
 * 集合原型方法集合。
 *
 * 提供集合原型上常用方法的类型描述，供 getProto 返回值使用。
 */
 ...
 * @returns 对象的原型
 */
const getProto = <T extends CollectionTypes>(v: T): CollectionProto =>
    Reflect.getPrototypeOf(v) as CollectionProto;
...
/**
 * Map 类型（统一 Map 与 WeakMap 的可调用方法签名）。
 *
 * 包括 Map 和 WeakMap。键参数放宽为 unknown，运行时由各集合自身处理。
 */
type MapTypes = Target & {
    get: (key: unknown) => unknown
    has: (key: unknown) => boolean
    set: (key: unknown, value: unknown) => MapTypes
    delete: (key: unknown) => boolean
    forEach?: (cb: (value: unknown, key: unknown) => void) => void
};

/**
 * Set 类型（统一 Set 与 WeakSet 的可调用方法签名）。
 *
 * 包括 Set 和 WeakSet。值参数放宽为 unknown，运行时由各集合自身处理。
 */
type SetTypes = Target & {
    has: (value: unknown) => boolean
    add: (value: unknown) => SetTypes
 ...
 * 可迭代集合类型。
 *
 * 包括 Map 和 Set，提供 forEach 与迭代器方法。
 */
type IterableCollections = Target & {
    size: number
    forEach: (cb: (value: unknown, key: unknown) => void) => void
    keys: () => Iterator<unknown>
    values: () => Iterator<unknown>
    entries: () => Iterator<[unknown, unknown]>
    [Symbol.iterator]: () => Iterator<[unknown, unknown]> | Iterator<unknown>
    clear: () => void
    get?: (key: unknown) => unknown
    has: (key: unknown) => boolean
    set?: (key: unknown, value: unknown) => unknown
 ...
 */
type WeakCollections = MapTypes | SetTypes;
