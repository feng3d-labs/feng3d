...
describe('响应式/computed', () =>
{
    ...
    it('依赖顺序应与上次获取值时一致', () =>
    {
        ...
        const a = ref(0) as RefReactivity<number>;
        const b = computed(() =>
            a.value % 3 !== 0) as ComputedReactivity<boolean>;
        const c = computed(() =>
        {
            cSpy();
            if (a.value % 3 === 2)
            ...
            return 'cheap';
        }) as ComputedReactivity<string>;
        const d = computed(() =>
            a.value % 3 === 2) as ComputedReactivity<boolean>;
        const e = computed(() =>
        {
            if (b.value)
            {
    ...
    {
        const v = ref(1);
        const c1 = computed(() => v.value) as Computed<number>;
        const c2 = computed(() => c1.value) as Computed<number>;
    ...
    it('computed 应在 untracked 区域强制追踪', () =>
    {
        ...
        effect(() =>
        {
            ...
            {
                n.value;
                c = computed(() => n.value + 1) as Computed<number>;
                // 立即访问 computed 强制刷新
                c.value;
                effect(() => spy2(c.value));
                n.value;
    ...
    test('从深度嵌套 computed 移除依赖时的性能', () =>
    {
        ...
        for (let i = 0; i < LAYERS; i++)
        {
            ...
            computeds.push(
                computed(() =>
                    base.value + earlier.reduce((sum, c) => sum + c.value, 0)) as Computed<number>,
            );
        }
