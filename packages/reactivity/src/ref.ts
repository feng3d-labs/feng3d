...
export function ref<T>(value?: T): Ref<T>
{
    ...
    return new RefReactivity<T>(value as T);
}
...
{
    return r
        ? (r as Record<PropertyKey, unknown>)[ReactiveFlags.IS_REF] === true
        : false;
}
 ...
 */
export interface RefReactivity<T = unknown> extends Ref<T> { }
 ...
 * 4. 依赖追踪
 */
export class RefReactivity<T = unknown> extends Reactivity<T> implements Ref<T>
{
    /**
     * 标识这是一个 ref 对象。
     *
 ...
 */
export interface Ref<T = unknown, S = T>
{
    get value(): T
    set value(_: S)
    [RefSymbol]: true
