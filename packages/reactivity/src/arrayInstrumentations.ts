...
)
{
    ...
    const iter = (arr[method] as () => IterableIterator<unknown>)() as IterableIterator<unknown> & {
 ...
 */
type ArrayMethods = keyof Array<unknown> | 'findLast' | 'findLastIndex';
...
)
{
    ...
    if (methodFn !== Array.prototype[method as keyof unknown[]])
    {
...
)
{
    ...
    return (arr[method] as (callbackfn: typeof fn, ...args: unknown[]) => unknown)(wrappedFn, ...args);
}
...
)
{
    ...
    const search = arr[method] as (...args: unknown[]) => unknown;
    // we run the method using the original args first (which may be reactive)
    const res = search.apply(arr, args);
...
)
{
    const res = batchRun(() =>
        noTrack(() =>
            (toRaw(self)[method] as (...args: unknown[]) => unknown).apply(self, args),
        ),
    );
