...
┌────────────────────────────▼────────────────────────────────────┐
│                       @feng3d/rendering                         │
│  GPU 剔除 • LOD 选择 • 命令生成 • 间接绘制                        │
└────────────────────────────┬────────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────────┐
│                       @feng3d/webgpu                            │
│  WebGPU 抽象 • 设备/缓冲/管线/着色器管理                         │
└─────────────────────────────────────────────────────────────────┘
```

**设计原则**: 单向依赖、职责单一、可替换后端
...
### @feng3d/webgpu
- ✅ 设备管理、缓冲/纹理/采样器创建、管线管理、命令编码
- ❌ 不负责场景管理、渲染算法、高层渲染策略

### @feng3d/rendering
- ✅ 接收预处理数据、GPU 视锥/遮挡剔除、LOD 选择、深度排序、命令生成、间接绘制
- ❌ 不负责场景图、组件系统、矩阵更新（由调用者计算好 modelMatrix）、资源加载

### @feng3d/core
- ✅ 场景图、组件系统、变换更新、包围盒计算、资源管理
