...
**单一全局时间源。** 引擎持有一个时间数据源 `{ t }`，由 View.update 每帧 `+= dt` 写入；依赖时间的 computed（动画插值、`u_time` 材质、粒子推进）每帧随之失效。

- 这是 4.1 白名单中"时间源"的具体形态：**动画运行 = 每帧有合法数据变化**，因此每帧重算相关链路是预期行为，与 G2 不冲突——G2 的"无修改零运算"只承诺无变化时。
- 时间控制是**数据**：`timeScale` / `paused` 字段由时间源 Logic 读取，暂停即停止写 `t`（下游自动静止），倍速即 `dt * timeScale`。
- **分频订阅**（动画 60Hz、阴影 15Hz 等降频更新）**不进第一版语义**——单一时间源保证失效语义最简单，降频作为后续优化项，在 benchmark 证明需要后再设计。

**动画的两种形态：**

- **终态：声明式动画。** `AnimationClip` / `PropertyClip` 作为纯数据，动画值为 computed：`(clip, t) → 插值结果`，完全惰性，时间变化只失效真正依赖该属性的下游。时间轴 seek（改 `t` 或 clip 时间字段）天然工作。
- **过渡：命令式动画。** 现有 `Animation.ts` 每帧写回 rotation 等字段，归入命令式逃生舱（4.3），标注为待迁移——它已经是合法的变更源，不阻塞阶段 1，但在声明式动画落地后由其取代。
