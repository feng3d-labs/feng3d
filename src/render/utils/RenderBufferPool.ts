module feng3d
{
    ...
    function getWebGLProgram(gl: GL, vertexCode: string, fragmentCode: string)
    {
        ...
        if (!linked)
        {
            ...
            engine.debuger && console.error('Failed to link program: ' + error + `\n${vertexCode}\n${fragmentCode}`);
    ...
    function getVertexShader(gl: GL, vertexCode: string)
    {
        ...
        if (shader == null)
        {
            engine.debuger && console.error('unable to create shader');
            return null;
        }
    ...
    function compileShader(gl: GL, shader: WebGLShader, shaderCode: string)
    {
        ...
        if (!compiled)
        {
            ...
            engine.debuger && console.error(`编译渲染程序时发生错误: ${error} \n ${gl.getShaderInfoLog(shader)}\n${shaderCode}`);
