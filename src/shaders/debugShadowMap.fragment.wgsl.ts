 ...
 * 用 textureLoad 读取 depth 纹理的深度值（[0,1]），可视化输出为灰度。
 * texture_depth_2d 只支持 textureSampleCompare / textureLoad，不支持 textureSampleLevel。
 *
 * 深度可视化：clearValue=1.0（无物体的区域为白色），有物体的区域深度较小（偏暗）。
 *
 * 绑定约定（与 TextureMaterial 一致）：
 * - @group(0) @binding(3) material_uniforms - { u_texSize: vec2 }
 * - @group(1) @binding(0) s_textureSampler: sampler（占位）
...
fn main(input: FragmentInput) -> FragmentOutput {
    ...
    // WebGPU 纹理 V=0 在顶部，翻转 Y 使画面正向显示
    ...
    // 可视化：深度直接作为灰度。
    // clearValue=1.0 → 白色（无物体），物体区域深度小 → 偏暗。
