...
export class MouseRay3D extends Component3D
{
    ...
    private _onBeforeRender(event: IEvent<RenderContext3D>)
    {
        ...
        const localRay = this.node3d.globalInvertMatrix.transformRay(globalRay);

        // 本地空间摄像机坐标
        const cameraLP = this.node3d.globalInvertMatrix.transformPoint3(localRay.origin);
        // 本地空间摄像机方向
        const cameraLD = this.node3d.globalInvertMatrix.transformVector3(localRay.direction);
        cameraLD.normalize();
