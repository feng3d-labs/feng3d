import { AddComponentMenu, Camera, createNodeMenu, Geometry, Material, Object3D, QuadGeometry, RegisterComponent, Renderable, RunEnvironment, Scene } from '@feng3d/core';
...
export class ParticleSystem extends Renderable
{
    ...
    update(interval: number)
    {
        ...
        // 粒子系统位置
        emitInfo.currentWorldPos.copy(this.object3D.worldPosition);
    ...
    beforeRender(renderAtomic: RenderAtomic, scene: Scene, camera: Camera)
    {
        ...
        if (isBillboard)
        {
            const cameraMatrix = camera.object3D.globalMatrix.clone();
            ...
            {
                localCameraForward = this.object3D.globalRotationInvertMatrix.transformPoint3(localCameraForward);
                localCameraUp = this.object3D.globalRotationInvertMatrix.transformPoint3(localCameraUp);
            }
    ...
    private _simulationSpaceChanged()
    {
        ...
        {
            const globalInvertMatrix = this.object3D.globalInvertMatrix;
    ...
    addParticlePosition(particle: Particle, position: Vector3, space: ParticleSystemSimulationSpace, name?: string)
    {
        ...
        if (space !== this.main.simulationSpace)
        {
            ...
            {
                this.object3D.globalInvertMatrix.transformPoint3(position, position);
            }
            else
            {
                this.object3D.globalMatrix.transformPoint3(position, position);
            }
        }
        //
    ...
    removeParticlePosition(particle: Particle, name: string)
    {
        ...
        if (obj)
        {
            ...
            if (space !== this.main.simulationSpace)
            {
                if (space === ParticleSystemSimulationSpace.World)
                {
                    this.object3D.globalInvertMatrix.transformPoint3(value, value);
                }
                else
                {
                    this.object3D.globalMatrix.transformPoint3(value, value);
                }
            }
            //
    ...
    addParticleVelocity(particle: Particle, velocity: Vector3, space: ParticleSystemSimulationSpace, name?: string)
    {
        ...
        if (space !== this.main.simulationSpace)
        {
            if (space === ParticleSystemSimulationSpace.World)
            {
                this.object3D.globalInvertMatrix.transformVector3(velocity, velocity);
            }
            else
            {
                this.object3D.globalMatrix.transformVector3(velocity, velocity);
            }
        }
        //
    ...
    removeParticleVelocity(particle: Particle, name: string)
    {
        ...
        if (obj)
        {
            ...
            if (space !== this.main.simulationSpace)
            {
                if (space === ParticleSystemSimulationSpace.World)
                {
                    this.object3D.globalInvertMatrix.transformVector3(value, value);
                }
                else
                {
                    this.object3D.globalMatrix.transformVector3(value, value);
                }
            }
            //
    ...
    addParticleAcceleration(particle: Particle, acceleration: Vector3, space: ParticleSystemSimulationSpace, name?: string)
    {
        ...
        if (space !== this.main.simulationSpace)
        {
            if (space === ParticleSystemSimulationSpace.World)
            {
                this.object3D.globalInvertMatrix.transformVector3(acceleration, acceleration);
            }
            else
            {
                this.object3D.globalMatrix.transformVector3(acceleration, acceleration);
            }
        }
        //
    ...
    TriggerSubEmitter(subEmitterIndex: number, particles: Particle[] = null)
    {
        ...
        particles.forEach((particle) =>
        {
            ...
            // 粒子所在世界坐标
            const particleWoldPos = this.object3D.globalMatrix.transformPoint3(particle.position);
            // 粒子在子粒子系统的坐标
            const subEmitPos = subEmitter.object3D.globalInvertMatrix.transformPoint3(particleWoldPos);
