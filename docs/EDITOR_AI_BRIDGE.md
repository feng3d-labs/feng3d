...
- **极端非法输入组合后可能栈溢出**：连续轰入多组非法参数（超范围颜色、空名字对象、多次复制
  与圆周排列混在一起）之后，环境设置与撤销会报 `Maximum call stack size exceeded`。此时刷新
  页面即可恢复，正常使用不会触发。桥接已对可预见的非法输入逐项加防（场景根 / 重复项 /
  类型 / NaN / 几何参数），模糊测试脚本见 `.verify/fuzz-probe.mjs`
...
> ⚠ 它会把页面轰到"环境设置与撤销栈溢出"的状态（见 §10 的已知限制），**跑完请刷新编辑器页面**。
